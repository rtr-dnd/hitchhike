...
public class InterRegionalTaskManager : MonoBehaviour
{
    ...
    public List<GameObject> regionAreas = new List<GameObject>(); // 7 areas with BoxColliders
    ...
    void Start()
    {
        ...
        if (regionAreas.Count != 7)
        {
            Debug.LogError($"Expected 7 region areas, but got {regionAreas.Count}!");
            return;
        }
    ...
    void SelectRandomTranslationAndRotation()
    {
    ...
    void GenerateAllExperimentalConditions()
    {
        ...
        // Generate all 42 combinations (7 start regions × 6 target regions)
    ...
    void SpawnObjectsForCondition()
    {
        ...
        Debug.Log($"Condition {currentConditionIndex}/{conditionIndices.Count}: Start Region={condition.startRegionIndex}, Target Region={condition.targetRegionIndex}, Translation={condition.translationAxis}, Rotation={condition.rotationAxis}");

        // Get region centers
        Vector3 startRegionCenter = GetRegionCenter(condition.startRegionIndex);
        Vector3 targetRegionCenter = GetRegionCenter(condition.targetRegionIndex);

        // Place movable object at start region center
        if (movableObject != null)
        {
            movableObject.transform.position = startRegionCenter;
            movableObject.transform.rotation = Quaternion.identity;
        }

        // Calculate target position and rotation
        Vector3 targetPosition = CalculateTargetPosition(targetRegionCenter, condition.translationAxis);
        Quaternion targetRotation = CalculateTargetRotation(condition.rotationAxis);
    ...
    Vector3 GetRegionCenter(int regionIndex)
    {
    ...
    Vector3 CalculateTargetPosition(Vector3 basePosition, TranslationAxis axis)
    {
    ...
    Quaternion CalculateTargetRotation(RotationAxisPair axisPair)
    {
        Vector3 rotationAxis = Vector3.zero;
        float angle = 45f; // Fixed at 45 degrees
    ...
    void ExportTaskTimesToCSV()
    {
        ...
        csv.AppendLine("Task Number,Time (seconds),Clutching Count,Start Region,Target Region,Translation Axis,Rotation Axis");
        ...
        for (int i = 0; i < taskTimes.Count; i++)
        {
            if (i < completedConditions.Count)
            {
                ...
                csv.AppendLine($"{i + 1},{taskTimes[i]:F3},{clutchingCounts[i]},{condition.startRegionIndex},{condition.targetRegionIndex},{condition.translationAxis},{condition.rotationAxis}");
            }
            else
            {
                csv.AppendLine($"{i + 1},{taskTimes[i]:F3},{clutchingCounts[i]},N/A,N/A,N/A,N/A");
            }
        }
