...
namespace AmazingAssets.RenderMonster
{
    public class RenderMonsterKeyCodeWindow : EditorWindow
    {
...
#if ENABLE_INPUT_SYSTEM
        public delegate void UpdateKeyCode(Key keyCode);
#else
        ...
        static public void ShowWindow(Vector2 position, UpdateKeyCode function)
        {
            ApplyKeyCode = function;

            window = (RenderMonsterKeyCodeWindow)RenderMonsterKeyCodeWindow.CreateInstance(typeof(RenderMonsterKeyCodeWindow));

            window.Focus();
            window.ShowAsDropDown(new Rect(position.x, position.y, 0, 0), new Vector2(250, 45));
        }
        ...
        void OnGUI()
        {
            ...
            if (Event.current != null)
            {
                ...
                {
#if ENABLE_INPUT_SYSTEM
                    ApplyKeyCode(KeyCodeToKey(Event.current.keyCode));
#else
